...
const BUILT_IN: Record<string, { icon: React.ReactNode; tone: string }> = {
  "Essential Docs": { icon: <IdCardIcon className="w-5 h-5" />, tone: "from-[#2563EB] to-[#4F46E5]" },
  "Bank Docs": { icon: <BankIcon className="w-5 h-5" />, tone: "from-[#16A34A] to-[#0D9488]" },
  "Vehicle": { icon: <CarIcon className="w-5 h-5" />, tone: "from-[#F59E0B] to-[#EA580C]" },
  "Non Essential Docs": { icon: <FolderIcon className="w-5 h-5" />, tone: "from-[#64748B] to-[#334155]" },
};

export function CategoryIcon({ name, emoji, size = "md" }: { name: string; emoji?: string; size?: "sm" | "md" }) {
  const built = BUILT_IN[name];
  const box = size === "sm" ? "w-7 h-7 rounded-lg text-sm" : "w-11 h-11 rounded-2xl text-lg";
  if (built) {
    return (
